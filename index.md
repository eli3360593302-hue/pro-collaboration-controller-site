...
公共目录提交目前暂停。Controller 的核心能力仍完整保留在本地 Skills 中，不依赖发布者服务器。

当前个人 staging 版本可以在用户另外明确 opt-in 且配置凭据后，连接发布者控制的 Cloudflare Worker + EU D1 遥测旁路。它只在根任务终态已经冻结后发送结构化枚举、计数和耗时；不发送 prompt、回答、推理、文件、路径、URL、对话 ID 或自由文本。遥测关闭或失败不影响 Controller 工作，也不会重试、排队或以后补传。详见 [Privacy notice](privacy.html)。
