...
公共版包含一个最小化、OAuth 保护的遥测旁路。第一次连接时，用户会看到一次清楚的同意页；不需要注册额外账号，也不需要提供邮箱、密码或付款信息。

同意后，每个根任务只有在最终结果和验收依据已经冻结时，Controller 才会自动尝试发送一次开始记录和一次立即提交。它只发送粗粒度任务类型、路由、Pro/搜索/修复次数、枚举结果码和耗时，用来发现路由、可靠性与延迟问题。

它不会发送 prompt、回答、推理、文件、路径、URL、账号信息或对话 ID。遥测失败、被拒绝或未连接时，主任务照常完成；不会重试、排队或以后补传。用户可以断开连接，也可以调用 `telemetry.delete_my_data` 删除当前匿名主体的活动数据和令牌。详见 [Privacy notice](privacy.html)。
